...
<html lang="en">
  <head>
    ...
    <meta name="application-name" content="RAVI MONITOR" />
    ...
    <meta property="og:site_name" content="RAVI MONITOR" />
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "WebApplication",
      "name": "RAVI MONITOR",
      "alternateName": ["RAVIMonitor", "RAVI Monitor App", "RM Intelligence"],
      "url": "${FULL.url}",
  ...
  <body>
    <h1 class="app-heading">RAVI MONITOR — Real-Time Global Intelligence Dashboard</h1>
...
describe('renderVariantDashboardHtml (#4996)', () => {
  ...
  it('rewrites brand meta, hreflang cluster, social images, and h1 for tech', () => {
    ...
    assert.ok(html.includes(`<meta property="og:site_name" content="${escHtml(tech.siteName)}" />`), 'og:site_name');
    assert.ok(html.includes(`<meta name="application-name" content="${escHtml(tech.siteName)}" />`), 'application-name');
  ...
  it('rewrites the WebApplication JSON-LD block but leaves the Organization block as World Monitor', () => {
    ...
    assert.equal(webApp.name, finance.siteName);
